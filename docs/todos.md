...
- [x] Replace the single shared MVP credential with real user records and hashed passwords
- [x] Add server-side session tracking and revocation instead of relying only on a signed username cookie
...
- [x] Make the day itinerary the default first impression for stakeholder demos instead of opening on the All Days board.
- [x] Add a pitch-ready trip overview with days planned, essentials covered, decisions needed, bookings missing, and latest activity.
- [x] Seed a complete realistic 6-day Vancouver/Whistler demo itinerary so empty days do not read as unfinished during stakeholder review.
- [x] Make the day navigation a sticky tab bar with horizontal scroll, selected state, card counts, and finalized badges.
- [x] Redesign Kanban cards for faster scanning: status chip, category chip, title, time/location line, truncated details, and footer actions.
...
- [x] Keep Ideas Inbox as the family idea inbox with category grouping, suggested-by visibility, and quick actions to assign, shortlist, or reject ideas.
...
- [x] Improve phone planning with a compact day-first layout, sticky day switcher, quick add, and tap-to-expand cards.
...
- [x] Combine or streamline initial authenticated loading for session, board, trip, and chat history.
- [ ] Profile React re-renders during drag/drop, day tab switching, quick-add, and AI board updates. (Observational — use React DevTools Profiler; no code change tracked here.)
- [x] Split heavy surfaces with lazy loading, especially AI chat and future map/readiness views.
- [ ] Move toward targeted board mutation endpoints before the board grows beyond MVP scale. (Large backend refactor — deferred.)
- [x] Add a repeatable Lighthouse or bundle-size check for production builds.