...
- **`api.py`** — All REST endpoints under `/api`. Key routes: `POST /api/login`, `GET/PUT /api/board`, `POST /api/chat`, `GET /api/chat/history`. Includes in-process rate limiting (sliding window, stored in `app.state.rate_limit_store`).
...
Next.js 16 app with static export (`output: "export"`). Uses `@dnd-kit` for drag-and-drop, Tailwind CSS for styling.

- **`app/`** — Next.js app router pages.
- **`components/`** — UI components (board, cards, columns, chat panel).
- **`lib/`** — API client, types, state management utilities.
